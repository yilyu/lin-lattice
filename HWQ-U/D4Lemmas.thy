(* D4 transition rule of system-invariant preservation proof *)
theory D4Lemmas
  imports
    Main
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI2_SSN_Bounds *)
(* ========================================================================= *)
...
proof -
  ...
  proof -
    from STEP obtain us_mid us_ret where u_steps: "U_D3 p (x_var s p) (s_var s p) (snd s) us_mid" "U_D4 p us_mid us_ret" "U_D5 p us_ret (snd s')"
      unfolding Sys_D4_def bridge by blast
    show "his_seq s' = his_seq s @ [mk_act deq (x_var s p) p (s_var s p) ret]"
      using u_steps unfolding U_D3_def U_D4_def U_D5_def bridge by auto
    show "program_counter s' p = ''L0''" "s_var s' p = s_var s p + 1"
      using STEP u_steps unfolding Sys_D4_def C_D4_def U_D3_def U_D4_def U_D5_def bridge
  ...
  proof -
    ...
    thus "program_counter s' q = program_counter s q" "s_var s' q = s_var s q"
      using STEP `q \<noteq> p` unfolding Sys_D4_def C_D4_def U_D3_def U_D4_def U_D5_def bridge
      by (auto simp: prod_eq_iff)
  qed
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation sI11_x_var_Scope *)
(* ========================================================================= *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI1_E_Phase_Pending_Enq *)
(* ========================================================================= *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI4_X_var_Lin_Sync *)
(* ========================================================================= *)
...
proof -
  ...
  have pc_E2_eq: "\<And>q. program_counter s' q = ''E2'' \<longleftrightarrow> program_counter s q = ''E2''"
    using STEP
    unfolding Sys_D4_def C_D4_def bridge
  ...
  have slots_eq: "E2SlotCount s' = E2SlotCount s"
    ...
    using i_var_eq apply simp
    done

  show ?thesis
    using inv_hI4_X_var_Lin_Sync X_var_eq lin_seq_eq slots_eq
    unfolding hI4_X_var_Lin_Sync_def LinEnqCount_def
    by auto
qed

(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI13_Qback_Deq_Sync (derivation version) *)
(* ========================================================================= *)
...
proof -
  {
    ...
    proof (cases "q_old = p")
      ...
      (* Use construct out of right_side fact, direct closure in of thesis! *)
      thus ?thesis using True by blast

    next
      case False
      (* ============================================================== *)
      (* It is!similarly, use directly have construct out corresponds to of complete of left or right real value *)
      (* ============================================================== *)
      ...
      (* Then use blast direct closure outside thesis of in! *)
      thus ?thesis by blast
    qed
  }
  (* 4. *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI14_Pending_Enq_Qback_Exclusivity (setdefinition version) *)
(* ========================================================================= *)
...
proof (intro conjI allI impI)
  (* ========================================================================= *)
  (* No. one: prove PC in E2, E3 of Qback_arr *)
  (* ========================================================================= *)
  fix q a
  ...
  (* Set derivation: provenew state of PendingEnq necessarily old state *)
  have "HasPendingEnq s q a"
  proof -
    (* 1. unfold Let_def, and match of *)
    ...
    (* 5. new old state of HasPendingEnq(then unfold Let_def) *)
    show ?thesis
...
next
  (* ========================================================================= *)
  (* No. two: prove PC in E1 of Qback_arr *)
  (* ========================================================================= *)
  fix q a
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI15_Deq_Result_Exclusivity (version) *)
(* ========================================================================= *)
...
proof -
  (* ========================================================================= *)
  (* Inside 1: DeqRet of sourcesplit (in set of version) *)
  (* Prove: new state in of DeqRet, in old history, then is when before p of x_var *)
  (* ========================================================================= *)
  ...
  (* ========================================================================= *)
  (* Inside 2: has of collapse (no blast version) *)
  (* Prove: new state of {history has \<or> inside has }, necessarilycorresponds toold state of similarly has *)
  (* ========================================================================= *)
...
(* ========================================================================= *)
  (* Derivation: have prefix, last make blast one *)
  (* ========================================================================= *)

  (* Part 1 fact: process for one value of has *)
  ...
  proof (intro allI impI notI)
    fix a p1 p2 sn1 sn2 :: nat (* <--- core change: in here local *)
...
(* Part 2 fact: in dequeue(D4) of in of value, definitely in Q_arr in *)
  ...
  proof (intro allI impI notI)
    ...
    (* In sn fact, new of setdefinition *)
    have sn_eq: "s_var s' q = s_var s q" using q_not_p s_var_other by simp

    (* Definition, use of set translate *)
    have pending_q: "HasPendingDeq s q"
    proof -
      (* 1. splitnew state of Pending fact *)
      ...
      (* 2. listappend as set of insert *)
      have set_s': "set (his_seq s') = insert (mk_act deq (x_var s p) p (s_var s p) ret) (set (his_seq s))"
        using his_append by auto

      (* 3. prove Call in old state: becauseappend of element in p, and we need q, in old set in *)
      ...
      (* 4. prove Ret in old state: newset all has, old set impossible has *)
      have no_ret_s: "\<forall>e\<in>set (his_seq s). act_ssn e = s_var s q \<longrightarrow> act_pid e = q \<longrightarrow> act_cr e \<noteq> ret"
        using no_ret_s' sn_eq set_s' by auto

      (* 5. as old state of HasPendingDeq *)
      show ?thesis
  ...
  (* Part 3 fact: already history of returnvalue, physicalarray Q_arr to it *)
  ...
  proof (intro allI impI)
    fix q a sn k :: nat (* <--- core change: in here local *)
  ...
  (* ========================================================================= *)
  (* : three fact, blast Model. prefixmapping *)
  (* ========================================================================= *)
  show ?thesis
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI16_BO_BT_No_HB (goal_cases one version) *)
(* ========================================================================= *)
...
proof (unfold hI17_BT_BT_No_HB_def Let_def, intro allI impI, goal_cases)
  (* Use goal_cases goal in of a and b, as nat *)
  case (1 a b)
  ...
  (* 2. Happens-Before equivalenceprove (in) *)
  have HB_eq: "HB_EnqRetCall s' a b = HB_EnqRetCall s a b"
    unfolding HB_EnqRetCall_def HB_Act_def
  proof
    (* : s' -> s *)
  ...
  (* 3. extract the old stateconclusion and goal *)
  have "\<not> HB_EnqRetCall s a b"
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI18_Idx_Order_No_Rev_HB (one version - HB fix) *)
(* ========================================================================= *)
...
proof (unfold hI18_Idx_Order_No_Rev_HB_def Let_def, intro allI impI, goal_cases)
  ...
  (* --- Proof step: prove for arbitrary x, y all into of HB equivalence --- *)
  ...
  (* 3. provecore: use and InQ s' of mapping s *)
  (* Here of 1 InQBack s' a, InQBack s' b and Idx s' a < Idx s' b *)
  hence stable_pre: "InQBack s a \<and> InQBack s b \<and> Idx s a < Idx s b"
    using 1 idx_stable inq_stable by simp

  (* From old stateinvariant in extract \<not> HB_EnqRetCall s b a *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI19_Scanner_Catches_Later_Enq (precise alignversion) *)
(* ========================================================================= *)
lemma hI19_Scanner_Catches_Later_Enq_D4_step:
  assumes inv_hI19_Scanner_Catches_Later_Enq: "hI19_Scanner_Catches_Later_Enq s"
    ...
    and inqback_stable: "\<And>x. InQBack s' x = InQBack s x"  (* Mustfill in of translate *)
...
proof (unfold hI19_Scanner_Catches_Later_Enq_def, intro allI impI, goal_cases)
  ...
  (* 3. when before D4 state transition of process p: q in D3, and p into L0 *)
  ...
  (* A. oldelement a, b of physicalmapping *)
  ...
  (* C. derivation HasPendingDeq (D4 append of is p of ret record, q \<noteq> p of start) *)
  ...
  (* 5. old guards hI19_Scanner_Catches_Later_Enq, allprecondition one in! *)
  have "\<not> HB_EnqRetCall s a b"
  ...
  (* 6. close immediatelynew state of goal *)
  then show "\<not> HB_EnqRetCall s' a b" using hb_stable by simp
qed

(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI21_Ret_Implies_Call (if-else version) *)
(* ========================================================================= *)
...
proof (unfold hI21_Ret_Implies_Call_def Let_def, intro allI impI, goal_cases)
  ...
  next
    case new_event
    (* --- branch two: newappend of deq ret --- *)
    ...
    proof -
      ...
      ultimately show ?thesis
        ...
        (* Here else branch, match BOT = BOT *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI22_Deq_Local_Pattern (fix D4 clearphysical factversion) *)
(* ========================================================================= *)
lemma hI22_Deq_Local_Pattern_D4_step:
  assumes inv_hI22_Deq_Local_Pattern: "hI22_Deq_Local_Pattern s"
    ...
    and inv_hI26_DeqRet_D4_Mutex: "hI26_DeqRet_D4_Mutex s"   (* !!!new: process!!! *)
    and his_append: "his_seq s' = his_seq s @ [mk_act deq (x_var s p) p (s_var s p) ret]"
    and pc_D4: "program_counter s p = ''D4''"
    and last_call_fact: "last (filter (\<lambda>e. act_pid e = p) (his_seq s)) = mk_act deq BOT p (s_var s p) call"
    and step_facts: (* Proof step: D4 clear this of physical fact!!! *)
        "(\<And>q. q \<noteq> p \<Longrightarrow> x_var s' q = x_var s q)"
        "x_var s' p = BOT"
        "(\<And>k. Q_arr s' k = Q_arr s k)"
        "(\<And>k. Qback_arr s' k = Qback_arr s k)"
        "(\<And>q. q \<noteq> p \<Longrightarrow> s_var s' q = s_var s q)"   (* !!!: q \<noteq> p!!! *)
...
proof (unfold hI22_Deq_Local_Pattern_def Let_def, intro allI impI, goal_cases)
  ...
  proof cases
    ...
    proof (intro allI)
      ...
      proof (cases "q' = p")
        case True
        (* 1. derivation v \<noteq> BOT, and prove v is has value (v \<in> Val) *)
        ...
        (* 2. old dequeuerecord of process q is is when before process p, *)
        show ?thesis
        proof (cases "q = p")
          case True_q: True
          (* In this process: use process hI26_DeqRet_D4_Mutex *)
        ...
        next
          case False_q: False
          (* In its process: use process hI15_Deq_Result_Exclusivity *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI23_Deq_Call_Ret_Balanced (localdequeueconsistency) *)
(* Version: mk_act of, of *)
(* ========================================================================= *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI24_HB_Implies_Idx_Order (empty consistency: HB array) *)
(* Version: complete event, only append of is deq, then enq of HB *)
(* ========================================================================= *)
...
proof (intro allI impI, goal_cases)
  (* Proof step: hI24_HB_Implies_Idx_Order_def in \<forall> of!!! *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI25_Enq_Call_Ret_Balanced (Enqueue operation of original) *)
(* Version: complete, only append of is deq event, and PC enter E1~E3 can *)
(* ========================================================================= *)
...
proof (intro allI impI)
  ...
  proof cases
    case old_k
    (* Branch one: k in old history inside *)
    have take_eq: "take k (his_seq s') = take k (his_seq s)"
      using old_k his_append by simp
  ...
  next
    case new_k
    (* Branch two: k append of event *)
    have take_all: "take k (his_seq s') = his_seq s'"
      using new_k by simp

    (* Core: D4 append of is deq, and only enq, thereforeneweventprecise! *)
    have filter_eq: "filter ?f_enq (his_seq s') = filter ?f_enq (his_seq s)"
      using his_append e_ret_is_deq by simp

    (* Its extract the old state of complete *)
    ...
    (* Prove in key of D4, all processes of all has in / E1~E3 *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation hI26_DeqRet_D4_Mutex (dequeuereturn of) *)
(* Version: complete event, useset direct closurehistory in equivalence *)
(* ========================================================================= *)
...
proof (intro allI impI)
  ...
  proof (cases "q = p")
    case True
    (* Core: when before process p already from D4 out, enter L0 *)
    have "program_counter s' p = ''L0''" using pc_p_s' by simp
    thus ?thesis using True by simp
  next
    case False
    (* For in its process, one old *)
    ...
    (* Core derivation: listappend set and, auto close the goal directly *)
    have deq_eq: "(\<exists>sn. DeqRetInHis s' q a sn) \<longleftrightarrow> (\<exists>sn. DeqRetInHis s q a sn)"
    proof -
      (* Appendlist as set of and *)
      have "set (his_seq s') = set (his_seq s) \<union> {e_ret}"
        using his_append by simp

      (* And fact, auto e_ret (because act_pid e_ret = p \<noteq> q) *)
      thus ?thesis
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation lI1_Op_Sets_Equivalence (operationset of complete) *)
(* Version: complete, use ret \<noteq> call, history *)
(* ========================================================================= *)
...
proof -
  (* 1. derivation: prove of call in D4 definitely *)
  (* No is nth is set, append of e_ret as ret, definitelyimpossiblematch call *)
  ...
  (* 2. use prove of fact, close immediatelyset of equal, for auto goal of *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation lI3_HB_Ret_Lin_Sync (consistency and complete) *)
(* Version: h2_old of linarith *)
(* ========================================================================= *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation lI7_D4_Deq_Deq_HB (Pending dequeue) *)
(* Physical: D4 append of is ret, new of call, new of HB *)
(* ========================================================================= *)
...
proof (goal_cases)
  case (1 k1 k2 q)
  (* At this point of premise:
     1. q in D4 (program_counter s' q = ''D4'')
     2. L! k2 is q of pending dequeueaction
     3. L! k1 and L! k2 HB
  *)
  ...
  (* 2.: old state q also in D4, and one *)
  ...
  (* 3. core: HB derivation (HB s' \<Longrightarrow> HB s) *)
    have hb_s: "HB (his_seq s) (lin_seq s ! k1) (lin_seq s ! k2)"
    proof -
      (* Here of 1 previously of goal_cases, it HB (his_seq s') (lin_seq s'! k1) (lin_seq s'! k2) *)
      have hb_raw: "HB (his_seq s') (lin_seq s' ! k1) (lin_seq s' ! k2)"
        using 1
        by auto

      (* Fixcore: the of HB_def obtain *)
      (* 1. only unfolding HB_def, unfolding match_... *)
      (* 2. where only HB_def in out of three *)
      ...
      (* 4. prove h2 in old history (h2 is newappend of e_ret) *)
      have h2_old: "h2 < length (his_seq s)"
      proof -
        (* Match_call definition in k < length H, extract it *)
        ...
        proof
          ...
          hence "his_seq s' ! h2 = e_ret" using his_append by (simp add: nth_append)
          hence "act_cr (his_seq s' ! h2) = ret" using e_ret_props by simp
          (* Contradiction: match_call act_cr is call *)
      ...
      (* 5. mappingmatch to old history s *)
      (* Use nth_append and match_ret/call of definition, prove in old history in match *)
      have m1: "match_ret (his_seq s) h1 (lin_seq s ! k1)"
  ...
  (* 4. old state of lI7_D4_Deq_Deq_HB *)
  show "k1 < k2"
  proof -
    (* Align lI7_D4_Deq_Deq_HB_list of *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation lI8_D3_Deq_Returned (D3 dequeuecomplete) *)
(* Use goal_cases, for failure of *)
(* ========================================================================= *)
lemma lI8_D3_Deq_Returned_D4_step:
  ...
  (* Here one completeall \<forall> and \<longrightarrow> *)
  apply (intro allI impI)
proof (goal_cases)
  (* Proof step: in here q and k with it of premise *)
  case (1 q k)
  (* As, we premise out *)
  ...
  proof -
    (* 1. q \<noteq> curr_p (because D3 \<noteq> L0) *)
    have "q \<noteq> curr_p"
    proof
      assume "q = curr_p"
      hence "program_counter s' q = ''L0'' " using pc_curr_p_s' by simp
    ...
    (* 2.: old state q also in D3 *)
    have pc_q_s: "program_counter s q = ''D3'' "
      using pc_q_s' \<open>q \<noteq> curr_p\<close> pc_other by auto

    (* 3. historyrecordequivalence: becauseappend of is curr_p of event, q of *)
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation lI9_D1_D2_Deq_Returned (D1/D2 dequeuecomplete) *)
(* Physical: D4 append of is curr_p of event, q (q\<noteq>curr_p) of history *)
(* ========================================================================= *)
...
proof (goal_cases)
  case (1 q k)
  (* At this point of premise:
     1. q is be process, k is
     2. program_counter s' q = ''D1'' \<or> program_counter s' q = ''D2''
     3. op_name (lin_seq s' ! k) = deq \<and> op_pid (lin_seq s' ! k) = q
  *)
  ...
  show "DeqRetInHis s' q (op_val (lin_seq s' ! k)) (op_ssn (lin_seq s' ! k))"
  proof -
    (* 1. q \<noteq> curr_p (because D1/D2 and L0) *)
    have "q \<noteq> curr_p"
    proof
      assume "q = curr_p"
      hence "program_counter s' q = ''L0'' " using pc_curr_p_s' by simp
    ...
    (* 2.: old state q also in D1 or D2 *)
    have pc_q_s: "program_counter s q = ''D1'' \<or> program_counter s q = ''D2'' "
      using pc_q_s' \<open>q \<noteq> curr_p\<close> pc_other by auto
...
(* ========================================================================= *)
(* Helper lemma: D4 step preservation lI10_D4_Enq_Deq_HB (Pending dequeue and historyenqueue of) *)
(* Physical: D4 append of is ret, new of call, HB preservemonotonicity *)
(* ========================================================================= *)
...
proof (goal_cases)
  case (1 k1 k2 q)
  (* 1. extractwhen before goal of premise: q in D4, k2 is its Pending action *)
  have pc_q_s': "program_counter s' q = ''D4'' " using 1 by simp

  (* 2. q \<noteq> curr_p (process p already D4 enter L0) *)
  have "q \<noteq> curr_p"
  proof
    assume "q = curr_p"
    hence "program_counter s' q = ''L0'' " using pc_curr_p_s' by simp
  ...
  (* 3.: old state q also in D4 *)
  have pc_q_s: "program_counter s q = ''D4'' "
    using pc_q_s' \<open>q \<noteq> curr_p\<close> pc_other by auto
  have vars_q: "x_var s' q = x_var s q" "s_var s' q = s_var s q"
    using \<open>q \<noteq> curr_p\<close> vars_other by auto

  (* 4. core: HB derivation (HB s' \<Longrightarrow> HB s) *)
  have hb_s: "HB (his_seq s) (lin_seq s ! k1) (lin_seq s ! k2)"
  proof -
    have hb_raw: "HB (his_seq s') (lin_seq s' ! k1) (lin_seq s' ! k2)"
      using 1
    ...
    (* Prove h2 (use action) in old history in *)
    have h2_old: "h2 < length (his_seq s)"
    proof -
      from \<open>match_call (his_seq s') h2 (lin_seq s' ! k2)\<close>
      have "h2 < length (his_seq s')" unfolding match_call_def by (simp add: Let_def)
    ...
    (* Mapping match to old history s *)
    have m1: "match_ret (his_seq s) h1 (lin_seq s ! k1)"
      using h1_old his_append \<open>match_ret (his_seq s') h1 (lin_seq s' ! k1)\<close> lin_eq
      unfolding match_ret_def by (simp add: Let_def nth_append)
...
proof -
  (* 0. extract of old invariantguards *)
  ...
  (* 1. extract D4 of physical fact,, of array *)
  note bridge = program_counter_def X_var_def V_var_def Q_arr_def
                Qback_arr_def i_var_def j_var_def l_var_def x_var_def v_var_def
                s_var_def lin_seq_def his_seq_def

  have his_eq: "his_seq s' = his_seq s @ [mk_act deq (x_var s p) p (s_var s p) ret]"
    using STEP unfolding Sys_D4_def U_D3_def U_D4_def U_D5_def bridge by auto
  ...
  have s_var_other: "\<And>q. q \<noteq> p \<Longrightarrow> s_var s' q = s_var s q"
    using STEP unfolding Sys_D4_def C_D4_def U_D3_def U_D4_def U_D5_def bridge by auto

  (* 2. derivation hI3_L0_E_Phase_Bounds of 5 large *)
  show ?thesis
  ...
  next
    ...
    proof (cases "q = p")
      ...
      proof -
        ...
        (* Proof step: in?p_his, unfold his_seq s *)
        ...
        proof -
          ...
          (* Provelastappend of element call of *)
        ...
        proof -
          ...
          (* Provelastappend of is call, ret of, therefore be filter *)
