...
(* Step 1 of the proof. *)
(* Prove the required intermediate property. Related symbols: take_append_le. *)
lemma take_append_le [simp]:
  "n \<le> length xs \<Longrightarrow> take n (xs @ ys) = take n xs"
  by simp

(* Original source location: approximately lines 3. *)
(* Prove the required intermediate property. Related symbols: nth_append_length. *)
lemma nth_append_length [simp]:
  "(xs @ [x]) ! (length xs) = x"
  by simp

(* Step 4 of the proof. *)
(* Prove the required intermediate property. Related symbols: length_append_singleton. *)
...
(* Step 6 of the proof. Related symbols: filter_append_distrib. *)
...
(* ========================================================================= *)
(* Prove the required intermediate property. Related symbols: data_independent. *)
(* ========================================================================= *)
...
(* Stability/equivalence argument. Related symbols: data_independent. *)
...
proof -
  ...
  (* Prove the required intermediate property. Related symbols: L1. *)
  have di1_eq: "data_independent L1 \<longleftrightarrow>
    ...
    apply (subst card_indices_eq_count_mset)+ (* Proof note. *)
    by simp
...
proof -
  ...
  (* Step 3: use the available invariant or hypothesis. Related symbols: data_independent. *)
  have "card ?S \<le> 1"
...
proof -
  ...
  (* Prove the required intermediate property. Related symbols: find_indices. *)
  show ?thesis
...
(* List-index and filtering reasoning. Related symbols: find_unique_index. *)
...
proof -
  (* Unfold the relevant definition and expose the required facts. Related symbols: find_unique_index. *)
  from assms(2) have
    "find_unique_index (\<lambda>a. op_name a = deq \<and> op_val a = x_val) L = Some k"
    by simp

  (* Definition block. Related symbols: find_unique_index. *)
  ...
  proof -
    ...
    (* Set, cardinality, and uniqueness reasoning. Related symbols: \<le>. *)
  ...
  (* List-index and filtering reasoning. Related symbols: find_indices. *)
...
proof (rule ccontr)
  ...
  (* Set, cardinality, and uniqueness reasoning. Related symbols: \<noteq>. *)
  ...
  (* Proof note. Related symbols: ?v. *)
  then have "card ?S \<le> 1" by auto

  (* Set, cardinality, and uniqueness reasoning. Related symbols: ?S, \<ge>. *)
...
lemma data_independent_append_deq_fresh:
  ...
  shows "data_independent (L @ [mk_op deq v p sn])" (* Key proof fix. *)
  unfolding data_independent_def
proof (intro conjI allI)
  (* Prove the required intermediate property. *)
  fix u
...
proof -
  ...
  (* Prove the required intermediate property. Related symbols: enq_indices, op_val. *)
  ...
  (* Prove the required intermediate property. Related symbols: enq_indices. *)
...
proof -
  ...
  (* Prove the required intermediate property. Related symbols: deq_indices, op_val. *)
  ...
  (* Prove the required intermediate property. Related symbols: deq_indices. *)
...
proof -
  (* Prove the required intermediate property. Related symbols: new_L. *)
  ...
  (* Unfold the relevant definition and expose the required facts. Related symbols: data_independent. *)
  show ?thesis
    unfolding data_independent_def
  proof (intro conjI allI)
    fix v
...
(* Enqueue-side reasoning. Related symbols: find_last_enq, l22. *)
...
proof
  ...
  (* Unfold the relevant definition and expose the required facts. Related symbols: find_last_enq. *)
  ...
  (* Prove the required intermediate property. Related symbols: b_act. *)
  ...
  (* Prove the required intermediate property. Related symbols: l22. *)
  show "op_name a = deq"
  proof (rule ccontr)
    ...
    (* Enqueue-side reasoning. Related symbols: l22, enq_indices, l2. *)
    from a_in_l22 and l22_equals have "a \<in> set (drop (last enq_indices + 1) l2)"
      by simp

    (* List-index and filtering reasoning. Related symbols: enq_indices, l2. *)
    then obtain k where
    ...
    (* Enqueue-side reasoning. Related symbols: op_name, enq_indices. *)
    ...
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: sorted_imp_sorted, last_ge. *)
    (* Enqueue-side reasoning. Related symbols: enq_indices. *)
    ...
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: last_ge, \<le>. *)
    ...
    (* Enqueue-side reasoning. Related symbols: \<ge>, enq_indices. *)
    with k_gt show False
      by linarith
  qed
qed

(* Enqueue-side reasoning. Related symbols: l1, SA, l2. *)
...
proof (intro allI impI)
  ...
  proof (rule ccontr)
    ...
    (* Enqueue-side reasoning. Related symbols: in_SA. *)
    ...
    (* Prove the required intermediate property. Related symbols: enq_idx, l1. *)
    (* List-index and filtering reasoning. Related symbols: find_last_SA, SA. *)
    (* List-index and filtering reasoning. Related symbols: SA, \<le>, find_last_SA. *)

    (* Prove the required intermediate property. Related symbols: enq_idx, SA. *)
    have enq_idx_in_SA: "enq_idx \<in> {j. j < length L \<and>
      op_name (L ! j) = enq \<and> in_SA (op_val (L ! j)) L}"
      using enq_props v_in_SA by auto

    (* Proof note. Related symbols: find_last_SA. *)
    (* List-index and filtering reasoning. Related symbols: find_last_SA. *)
    (* Definition block. Related symbols: find_last_SA. *)
    ...
    (* Enqueue-side reasoning. Related symbols: enq_idx, SA, sa_indices. *)
    have sa_indices_nonempty: "find_indices (\<lambda>a. op_name a = enq \<and> in_SA (op_val a) L) L \<noteq> []"
    proof -
      (* Unfold the relevant definition and expose the required facts. Related symbols: enq_idx_in_SA. *)
      ...
      (* Unfold the relevant definition and expose the required facts. Related symbols: find_indices. *)
      ...
      (* Prove the required intermediate property. Related symbols: enq_idx. *)
      have "enq_idx \<in> set [i \<leftarrow> [0..<length L]. (\<lambda>a. op_name a = enq \<and> in_SA (op_val a) L) (L ! i)]"
      proof -
        (* List-index and filtering reasoning. Related symbols: enq_idx. *)
        ...
        (* Enqueue-side reasoning. Related symbols: enq_idx. *)
        show ?thesis
    ...
    (* List-index and filtering reasoning. Related symbols: find_last_SA, find_indices. *)
    ...
    (* Enqueue-side reasoning. Related symbols: enq_idx, SA, sa_indices, \<le>. *)
    ...
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: last_sa_pos, find_last_SA. *)
    from assms(5) have last_sa_pos_eq: "last_sa_pos = find_last_SA L" by simp

    (* Enqueue-side reasoning. Related symbols: enq_idx, \<le>, last_sa_pos. *)
    ...
    proof -
      ...
      (* List-index and filtering reasoning. Related symbols: enq_idx, \<le>, find_indices, \<ge>. *)
    ...
    (* Proof note. Related symbols: l1, last_sa_pos. *)
    ...
    (* List-index and filtering reasoning. Related symbols: enq_idx, l1. *)
    ...
    (* List-index and filtering reasoning. Related symbols: \<ge>, l1, enq_idx. *)
...
proof (intro allI iffI)
  ...
  proof -
    ...
    (* Enqueue-side reasoning. Related symbols: new_L. *)
    (* Prove the required intermediate property. Related symbols: new_L. *)
    ...
    proof (rule ccontr)
      ...
      (* Proof note. Related symbols: find_unique_index. *)
      then show False
    ...
    proof (rule ccontr)
      ...
      (* Dequeue-side reasoning. Related symbols: new_L. *)
      (* Contradiction argument. *)
      ...
      proof -
        ...
        (* List-index and filtering reasoning. Related symbols: new_L. *)
    ...
    (* Proof note. Related symbols: in_SA, new_L. *)
    show "in_SA v new_L"
  ...
  proof -
    ...
    (* Proof note. Related symbols: in_SA. *)
    show "in_SA v L"
      using deq_exists_L enq_exists_L in_SA_def by auto
  qed
qed

(* Proof note. Related symbols: l3, l1, SA. *)
lemma l3_elements_not_in_SA:
  ...
  shows "\<not> in_SA v L"
proof -
  (* Extract the required witnesses and facts. Related symbols: l3. *)
  ...
  (* Use the relevant invariant, lemma, or hypothesis. Related symbols: l1_contains_all_SA. *)
...
proof -
  ...
  proof -
    ...
    also have "... = find_indices ?P2 L2"
      unfolding find_indices_def
    proof (rule filter_cong) (* Use the relevant invariant, lemma, or hypothesis. Related symbols: filter_cong. *)
      (* Prove the required intermediate property. *)
...
lemma data_independent_implies_unique_deq:
  ...
  shows "i = j"
proof -
  (* Step 1 of the proof. Related symbols: data_independent. *)
  ...
  (* Step 4: prove the required intermediate property. *)
...
(* Linearization-sequence reasoning. Related symbols: lI1_Op_Sets_Equivalence, lI2_Op_Cardinality, SetA, in_SA, lin_seq. *)
...
proof
  let ?v = "op_val x"
  let ?L = "lin_seq s" (* Use the relevant invariant, lemma, or hypothesis. Related symbols: lin_seq. *)

  (* Direction 1 of the proof. Related symbols: SetA, in_SA. *)
  show "?v \<in> SetA s \<Longrightarrow> in_SA ?v ?L"
  proof -
    assume "?v \<in> SetA s"
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: lI2_Op_Cardinality, SetA. *)
    ...
    proof -
      (* Step 1: perform the set/cardinality reasoning. *)
      ...
      (* Step 4 of the proof. Related symbols: find_unique_index. *)
      (* Set, cardinality, and uniqueness reasoning. Related symbols: find_unique_index. *)
      (* Definition block. Related symbols: find_unique_index. *)
      have "\<exists>!i. i < length ?L \<and> op_name (?L ! i) = enq \<and> op_val (?L ! i) = ?v"
        using k_prop k_is_enq k_val k_unique
        by blast

      (* Step 5 of the proof. Related symbols: find_indices. *)
      (* Prove the required intermediate property. Related symbols: find_indices. *)
      ...
      (* Step 6 of the proof. *)
      then show ?thesis
        (* Unfold the relevant definition and expose the required facts. Related symbols: find_unique_index_def. *)
        (* Unfold the relevant definition and expose the required facts. Related symbols: Let_def. *)
    ...
    proof -
      ...
      (* Step 3 of the proof. Related symbols: find_indices. *)
      (* Prove the required intermediate property. Related symbols: find_indices. *)
      ...
      (* Step 4 of the proof. *)
      then show ?thesis
        (* Unfold the relevant definition and expose the required facts. Related symbols: find_unique_index_def, Let_def. *)
        unfolding find_unique_index_def Let_def
        by auto
    qed

    (* Definition block. Related symbols: in_SA. *)
    show "in_SA ?v ?L"
      unfolding in_SA_def
...
next
  (* Direction 2 of the proof. Related symbols: in_SA, SetA. *)
  (* Use the relevant invariant, lemma, or hypothesis. Related symbols: ?v. *)
  ...
  proof -
    ...
    (* Step 2 of the proof. Related symbols: in_SA. *)
    (* Proof note. Related symbols: in_SA. *)
    (* Definition block. Related symbols: in_SA, find_unique_index. *)
    have deq_exists: "find_unique_index (\<lambda>a. op_name a = deq \<and> op_val a = ?v) ?L \<noteq> None"
      using asm unfolding in_SA_def
      by (auto split: option.splits if_splits)

    (* Step 3: extract the required facts. Related symbols: find_unique_index. *)
    ...
    proof -
      ...
      (* Use the relevant invariant, lemma, or hypothesis. Related symbols: find_indices. *)
      (* List-index and filtering reasoning. Related symbols: find_indices. *)
      ...
      (* Use the relevant invariant, lemma, or hypothesis. *)
      show ?thesis
        apply (rule that[of ?witness]) (* Proof note. *)
        using witness_in_L idx_prop_oper idx_prop_val by auto
    qed

    (* Step 4: use the available invariant or hypothesis. Related symbols: lI1_Op_Sets_Equivalence. *)
    (* Linearization-sequence reasoning. *)
    have "deq_act \<in> OPLin s"
      using deq_in_L unfolding OPLin_def by simp

    (* Step 5: prove the required intermediate property. Related symbols: OP_A_deq. *)
    ...
    proof -
      ...
      (* Enqueue-side reasoning. Related symbols: OP_A_enq. *)
      moreover have "\<forall>x \<in> OP_A_enq s. op_name x = enq"
        unfolding OP_A_enq_def
        (* Unfold the relevant definition and expose the required facts. Related symbols: mk_op_def, op_name_def, mk_op. *)
      ...
      (* Dequeue-side reasoning. Related symbols: deq_act, OP_A_deq. *)
      ultimately show ?thesis
        using `deq_act \<in> OPLin s` is_deq
        by auto
    qed

    (* Step 6 of the proof. Related symbols: OP_A_deq, SetA. *)
    (* Dequeue-side reasoning. Related symbols: OP_A_deq, mk_op, \<in>, SetA. *)
    show "?v \<in> SetA s"
      using `deq_act \<in> OP_A_deq s` val_is_v
      unfolding OP_A_deq_def
      (* Unfold the relevant definition and expose the required facts. Related symbols: op_val_def, mk_op_def, op_val, mk_op. *)
      by (auto simp add: op_val_def mk_op_def)
  qed
qed

(* ---------------------------------------------------------------- *)
(* Enqueue-side reasoning. Related symbols: SA, lI5_SA_Prefix. *)
(* Use the relevant invariant, lemma, or hypothesis. Related symbols: lI1_Op_Sets_Equivalence, SetA, in_SA. *)
(* ---------------------------------------------------------------- *)
...
proof (intro ballI impI)
  ...
  (* Step 2: use the available invariant or hypothesis. Related symbols: lI1_Op_Sets_Equivalence. *)
  have "x \<in> OPLin s" using x_in_L unfolding OPLin_def by simp

  (* Enqueue-side reasoning. Related symbols: A_enq, A_deq, B_enq. *)
  ...
  proof
    ...
    (* Dequeue-side reasoning. Related symbols: OP_A_deq, op_name. *)
    then have "op_name x = deq" unfolding OP_A_deq_def by blast
    then show False using prem by simp
  qed

  (* Step 4: use the available invariant or hypothesis. Related symbols: OP_A_enq, in_SA. *)
  have not_A_enq: "x \<notin> OP_A_enq s"
  proof
    assume "x \<in> OP_A_enq s"
    (* Key proof fix. Related symbols: mk_op. *)
    ...
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: SetA, in_SA. *)
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: SetA_equivalent_in_SA. *)
    have "in_SA ?v ?L"
      using SetA_equivalent_in_SA[OF assms(1) assms(2)] `?v \<in> SetA s` by simp

    (* Contradiction argument. Related symbols: \<not>, in_SA. *)
    then show False using prem by simp
  qed

  (* Step 5 of the proof. Related symbols: OP_B_enq. *)
  from x_union not_A_deq not_A_enq have "x \<in> OP_B_enq s" by blast

  (* Enqueue-side reasoning. Related symbols: active_enqs, OP_B_enq. *)
...
(* Auxiliary proof material. Related symbols: TypeB. *)
lemma TypeBT_implies_TypeB:
  "TypeBT s a \<Longrightarrow> TypeB s a"
  unfolding TypeBT_def by simp

(* Auxiliary proof material. Related symbols: SetB. *)
...
lemma card_DeqIdxs_mset_unchanged:
  assumes mset_eq: "mset (lin_seq s') = mset (lin_seq s) + {# act #}"
  assumes diff_val: "op_val act \<noteq> a" (* Proof note. *)
...
proof -
  ...
  (* Step 3 of the proof. Related symbols: op_val. *)
...
proof -
  ...
  (* ========================================================== *)
  (* Step 1: prove the required intermediate property. Related symbols: L1, r1, r2. *)
  (* ========================================================== *)
  have "r1 < r2"
  proof -
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: i1, j1. *)
    have "Suc i1 \<le> j1" using idx1(1) by simp
    then obtain k where k_def: "j1 = Suc i1 + k"
      using le_Suc_ex by blast

    (* List-index and filtering reasoning. Related symbols: j1, L1. *)
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: i1, L1, idx1. *)
    ...
    (* List-index and filtering reasoning. Related symbols: i1. *)
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: idx1, i1, j1, L1. *)
  ...
  proof -
    ...
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: append_take_drop_id, L1. *)
    (* Use the relevant invariant, lemma, or hypothesis. Related symbols: filter_append. *)
    have split_L1: "filter P L1 = filter P (take (Suc i1) L1) @ filter P (drop (Suc i1) L1)"
      by (metis append_take_drop_id filter_append)

    (* Step 2 of the proof. Related symbols: i1. *)
    have take_Suc: "take (Suc i1) L1 = take i1 L1 @ [L1 ! i1]"
      using idx1 by (simp add: take_Suc_conv_app_nth)

    (* Step 3 of the proof. Related symbols: P1, i1. *)
    ...
    (* Step 5: prove the required intermediate property. *)
    (* List-index and filtering reasoning. Related symbols: r1. *)
  ...
  (* Prove the required intermediate property. Related symbols: r2, L1, j1. *)
  ...
  (* ========================================================== *)
  (* Step 3: use the available invariant or hypothesis. Related symbols: L2. *)
  (* ========================================================== *)

  (* Proof note. Related symbols: L2, i2, r1. *)
  have r1_L2: "?rank L2 i2 = r1"
  proof (rule ccontr)
    assume neq: "?rank L2 i2 \<noteq> r1"
    let ?r_i2 = "?rank L2 i2"

    (* Prove the required intermediate property. Related symbols: L2, ?r_i2, i2. *)
    have val_at_rank: "filter P L2 ! ?r_i2 = L2 ! i2"
    proof -
      (* Step 1 of the proof. Related symbols: L2. *)
      ...
      (* Step 4: prove the required intermediate property. Related symbols: rank_lt. *)
      have rank_lt: "?r_i2 < length (filter P (take (Suc i2) L2))"
        using filter_take_Suc by simp

      (* Step 1 of the proof. *)
      (* List-index and filtering reasoning. Related symbols: nth_append. *)
      show ?thesis
        using split_L2 local_nth rank_lt
        by (simp add: nth_append)
    qed

    (* Prove the required intermediate property. Related symbols: ?r_i2. *)
    ...
    (* Prove the required intermediate property. Related symbols: r1. *)
    have valid_r1: "r1 < length (filter P L2)"
    proof -
      (* Use the relevant invariant, lemma, or hypothesis. Related symbols: L2, L1. *)
      (* Prove the required intermediate property. Related symbols: r1, L1. *)
  ...
  (* Proof note. Related symbols: L2, j2, r2. *)
  have r2_L2: "?rank L2 j2 = r2"
  proof -
    let ?r_j2 = "?rank L2 j2"

    (* ========================================================== *)
    (* Prove the required intermediate property. Related symbols: L2, ?r_j2, j2. *)
    (* ========================================================== *)
    have val_at_rank: "filter P L2 ! ?r_j2 = L2 ! j2"
    proof -
      (* Step 1 of the proof. Related symbols: L2. *)
      (* L2 = take (Suc j2) ... @ drop (Suc j2) ... *)
      have split_L2: "filter P L2 = filter P (take (Suc j2) L2) @ filter P (drop (Suc j2) L2)"
        by (metis append_take_drop_id filter_append)

      (* Step 2 of the proof. Related symbols: j2. *)
      ...
      (* Step 4: prove the required intermediate property. Related symbols: nth_append. *)
    ...
    (* ========================================================== *)
    (* Prove the required intermediate property. Related symbols: r2. *)
    (* ========================================================== *)
    have valid_r2: "r2 < length (filter P L2)"
    proof -
      (* Step 1: use the available invariant or hypothesis. Related symbols: L1, len_L1_split. *)
      ...
      (* Step 2 of the proof. Related symbols: r2, len_prefix. *)
      ...
      (* Step 3 of the proof. Related symbols: r2. *)
      (* Use the relevant invariant, lemma, or hypothesis. Related symbols: len_L1_split, len_prefix. *)
      have "r2 < length (filter P L1)"
        using len_L1_split len_prefix by arith

      (* Step 4 of the proof. Related symbols: L2. *)
      thus ?thesis using eq by simp
    qed

    (* ========================================================== *)
    (* Prove the required intermediate property. Related symbols: ?r_j2. *)
    (* ========================================================== *)
    have valid_rj2: "?r_j2 < length (filter P L2)"
    proof -
      (* Step 1: use the available invariant or hypothesis. Related symbols: L2. *)
      ...
      (* Step 2 of the proof. Related symbols: ?r_j2. *)
  ...
  proof (rule ccontr)
    ...
    (* Proof note. Related symbols: j2, i2. *)
    (* Use the relevant invariant, lemma, or hypothesis. *)
    ...
    (* Proof note. Related symbols: r2, r1. *)
    thus False using r2_L2 r1_L2 `r1 < r2` by simp
  qed
qed

(* Auxiliary proof material. *)
(* List-index and filtering reasoning. Related symbols: L1, L2.
*)
...
proof -
  ...
  (* Step 2: prove the required intermediate property. Related symbols: L1, ?rank. *)
  ...
  (* Step 3: prove the required intermediate property. Related symbols: ?rank. *)
  ...
  (* Step 5 of the proof. Related symbols: L2. *)
  {
    fix k
    ...
    next
      ...
      show ?case
      proof (cases "P y")
        case True note Py = True (* Proof note. *)
        show ?thesis
        proof (cases "k = 0")
          case True
          (* Proof note. *)
        ...
        qed
      next
        case False note nPy = False (* Proof note. *)
        (* List-index and filtering reasoning. *)
  ...
  (* Proof note. Related symbols: ?rank. *)
...
proof
  ...
  (* Step 1 of the proof. *)
  (* Proof-engineering note: avoid expensive unfolding and unstable automation. Related symbols: find_indices. *)
  ...
  (* Step 4 of the proof. Related symbols: find_last_SA, ?indices. *)
  (* Definition block. *)
  have last_sa_eq: "find_last_SA L = int (last ?indices)"
    using not_empty unfolding find_last_SA_def Let_def
    by simp

  (* Step 5: prove the required intermediate property. Related symbols: ?indices, sorted_find_indices. *)
  (* List-index and filtering reasoning. *)
  have "sorted ?indices"
    unfolding find_indices_def
    by (simp add: sorted_wrt_filter)

  (* Step 6: prove the required intermediate property. Related symbols: ?indices, sorted_max. *)
  (* Use the relevant invariant, lemma, or hypothesis. Related symbols: rev_induct. *)
  ...
  (* Step 7 of the proof. *)
  (* Proof note. Related symbols: ?indices, find_last_SA. *)
...
(* Auxiliary proof material. Related symbols: find_unique_index. *)
...
(* Enqueue-side reasoning. Related symbols: find_last_enq. *)
...
proof -
  ...
  (* Prove the required intermediate property. Related symbols: last_idx. *)
  ...
  (* Prove the required intermediate property. Related symbols: enq_act. *)
  ...
  proof (rule ccontr)
    ...
    (* Prove the required intermediate property. Related symbols: ?abs_pos, enq_indices. *)
    (* Original source location. *)
    have abs_pos_in_indices: "?abs_pos \<in> set enq_indices"
    proof -
      (* Unfold the relevant definition and expose the required facts. Related symbols: enq_indices. *)
      have enq_indices_def_expanded:
        "enq_indices = [i \<leftarrow> [0..<length L]. op_name (L ! i) = enq]"
        by (simp add: indices_def find_indices_def)

      (* Prove the required intermediate property. Related symbols: ?abs_pos, op_name. *)
      have condition_holds: "op_name (L ! ?abs_pos) = enq"
        using L_at_abs_pos a_is_enq by auto

      (* List-index and filtering reasoning. Related symbols: ?abs_pos. *)
      have in_range: "?abs_pos \<in> set [0..<length L]"
        using abs_pos_lt_length by auto

      (* Proof note. Related symbols: ?abs_pos. *)
      show ?thesis
    ...
    (* List-index and filtering reasoning. Related symbols: ?abs_pos, enq_indices. *)
    have gt: "?abs_pos > last enq_indices"
      by simp

    (* Enqueue-side reasoning. Related symbols: enq_indices, abs_pos_in_indices, abs_pos, \<le>. *)
    ...
    proof -
      ...
      proof (intro ballI)
        ...
        (* Enqueue-side reasoning. Related symbols: enq_indices, \<le>. *)
...
proof -
  ...
  (* Step 5 of the proof. Related symbols: find_unique_index. *)
  (* List-index and filtering reasoning. Related symbols: find_unique_index. *)
  show ?thesis
    unfolding find_unique_index_def Let_def
    using len_1 by (auto)
...
proof -
  ...
  next
    ...
    (* Prove the required intermediate property. Related symbols: mid_list. *)
...
proof -
  ...
  next
    ...
    proof (cases "k < idx + m_len")
      case True
      (* Proof note. Related symbols: mid_list. *)
...
lemma matched_pair_implies_in_SA:
  ...
  shows "in_SA v L"
proof -
  (* Step 1: prove the required intermediate property. Related symbols: unique_enq_index, find_indices. *)
  ...
  (* Enqueue-side reasoning. Related symbols: find_unique_index, k_enq. *)
  ...
  (* Step 2: prove the required intermediate property. Related symbols: unique_deq_index, find_indices. *)
  ...
  (* Dequeue-side reasoning. Related symbols: find_unique_index, k_deq. *)
  ...
  (* Step 3 of the proof. Related symbols: in_SA. *)
  show ?thesis
    unfolding in_SA_def
...
proof -
  (* Happens-before reasoning. Related symbols: O2, HB, O1, k2, k1. *)
  ...
  (* Contradiction argument. Related symbols: k1, k2. *)
  thus ?thesis
...
(* Happens-before reasoning. Related symbols: HB, o1, o2. *)
(* ---------------------------------------------------------------- *)
lemma HB_timeline_chain:
  assumes "happens_before a o1 H"           (* a.ret < o1.call *)
  assumes "\<not> happens_before o2 o1 H"        (* o1.call <= o2.ret *)
  assumes "happens_before o2 b H"           (* o2.ret < b.call *)
  shows "happens_before a b H"              (* a.ret < b.call *)
proof -
  (* Happens-before reasoning. Related symbols: HB, o1. *)
  ...
  (* Happens-before reasoning. Related symbols: o2, HB. *)
  ...
  (* Step 3: prove the required intermediate property. Related symbols: o1, o2. *)
  have "k2_o1 \<le> k1_o2"
  proof (rule ccontr)
    assume "\<not> (k2_o1 \<le> k1_o2)"
    hence "k1_o2 < k2_o1" by simp

    (* Happens-before reasoning. Related symbols: o2, HB, o1. *)
    have "happens_before o2 o1 H"
      unfolding HB_def
      using `k1_o2 < k2_o1` o2_b(2) a_o1(3) by blast

    (* Contradiction argument. Related symbols: \<not>, happens_before, o2, o1. *)
    with assms(2) show False by contradiction
  qed

  (* Step 4 of the proof. Related symbols: a_ret, o1_call, o2_ret, b_call, \<Longrightarrow>. *)
  hence "k1_a < k2_b"
...
(* ===================================================================== *)
  ...
  (* ===================================================================== *)
  (* Prove the required intermediate property. Related symbols: ?R. *)
  (* ===================================================================== *)
  have "length (filter ?P ?R) > 0"
  proof -
    (* Contradiction argument. Related symbols: ?R. *)
    have "L ! kv \<in> set ?R"
    proof -
