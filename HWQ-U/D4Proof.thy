...
proof -
  ...
  have TypeOK_s: "TypeOK s" and sI1_Zero_Index_BOT_s: "sI1_Zero_Index_BOT s" and sI2_X_var_Upper_Bound_s: "sI2_X_var_Upper_Bound s"
   ...
   and hI21_s: "hI29_E2_Scanner_Immunity s" and hI22_s: "hI30_Ticket_HB_Immunity s" (* Extract hI22_s *)
  ...
  proof -
    ...
    show "Q_arr s' = Q_arr s" "Qback_arr s' = Qback_arr s" "v_var s' = v_var s" "i_var s' = i_var s"
      ...
      using STEP u_steps
      unfolding Sys_D4_def C_D4_def U_D3_def U_D4_def U_D5_def bridge
  ...
  (* HI2_SSN_Bounds: global SSN and L0 *)
  ...
  (* ========================================================================= *)
  (* SI11_x_var_Scope: global guards (only in D4, x_var as BOT) *)
  (* ========================================================================= *)
  have "sI11_x_var_Scope s'"
    using sI11_x_var_Scope_D4_step[OF sI11_x_var_Scope_s STEP] .

(* HI1_E_Phase_Pending_Enq: prove start of enqueueoperation in D4 appenddequeuerecord after start *)
  ...
  (* ========================================================================= *)
  (* HI4_X_var_Lin_Sync: physical- mappingconsistency (D4 state transition) *)
  (* ========================================================================= *)
  have "hI4_X_var_Lin_Sync s'"
    using hI4_X_var_Lin_Sync_D4_step[OF hI4_X_var_Lin_Sync_s STEP] .

  (* ------------------------------------------------------------------------- *)
  (* 5. history and invariantpreserve (hI, linI - List Append) *)
  (* ------------------------------------------------------------------------- *)
  ...
  qed
    ...
    proof (rule hI19_Scanner_Catches_Later_Enq_D4_step)
      ...
      (* Corefill in: alignnew of inqback_stable *)
      show "\<And>x. InQBack s' x = InQBack s x"
        using step_facts unfolding InQBack_def by auto
  ...
  (* ========================================================================= *)
  (* HI19: and physical PC of (D4 simplification stepversion) *)
  (* ========================================================================= *)
  ...
  (* ========================================================================= *)
  (* HI20: enqueuevalue of definitelynew (fixguard, E3) *)
  (* ========================================================================= *)
  ...
  qed
    ...
    (* ========================================================================= *)
    (* HI21: E2 phase process of (D4 state transitionfill in - version) *)
    (* ========================================================================= *)
    ...
    proof (unfold hI29_E2_Scanner_Immunity_def, intro allI impI, goal_cases)
      ...
      (* 2. extractglobalphysicalmapping and old guards *)
      have hI21_s: "hI29_E2_Scanner_Immunity s" using INV unfolding system_invariant_def by blast

      (* 3. isolate the process currently taking the transition p: because p in D4 after to L0 *)
      have p_enq_neq_p: "p_enq \<noteq> p"
      proof
        assume "p_enq = p"
      ...
      (* 4. physical translate old state s *)
      ...
      (* Coretranslate: D4 append p of deq ret, but for q_deq \<noteq> p of Pending no *)
      ...
      (* 5. old stateguardsdirect closure! 7 precise *)
      ...
      (* 6. prove HB in append deq ret preserve, close immediately *)
      show "\<not> HB_EnqRetCall s' a (v_var s' p_enq)"
      proof -
        (* Use the of append HB_enq_stable_deq_append *)
...
(* ========================================================================= *)
    (* HI22: physical of definitelyorder preservation (D4 state transition - newversionsimplified definition) *)
    (* : TypeB, use append prove in dequeuereturn of definitely *)
    (* ========================================================================= *)
    ...
    proof (unfold hI30_Ticket_HB_Immunity_def, intro allI impI, goal_cases)
      ...
      (* 3. isolate the process currently taking the transition p: p in D4 after enter L0, impossible is E2/E3 of q *)
      have q_neq_p: "q \<noteq> p"
      proof
        assume "q = p"
      ...
      (* Coretranslate: prove HB (append deq ret definitely enq) *)
      have seq_eq: "his_seq s' = his_seq s @ [mk_act deq (x_var s p) p (s_var s p) ret]"
        using step_facts by simp

      (* Invariant: use auto replace of metis! append one! *)
      ...
      (* 5. precisemapping old state, versionguards! *)
  ...
  (* ------------------------------------------------------------------------- *)
  (* 5.5. USpec invariant uI1/uI2/uI3 in D4 preserve *)
  (* ------------------------------------------------------------------------- *)

  have uspec_effOps_eq [simp]: "uspec_effOps s' = uspec_effOps s"
  proof -
    from STEP obtain us_mid us_ret where
      u_steps: "U_D3 p (x_var s p) (s_var s p) (snd s) us_mid"
               "U_D4 p us_mid us_ret"
               "U_D5 p us_ret (snd s')"
      unfolding Sys_D4_def bridge by blast
    thus ?thesis
      unfolding U_D3_def U_D4_def U_D5_def uspec_effOps_def
      by auto
  qed

  have u_pc_other [simp]: "\<And>q. q \<noteq> p \<Longrightarrow> u_program_counter (snd s') q = u_program_counter (snd s) q"
  proof -
    fix q
    assume qp: "q \<noteq> p"
    from STEP obtain us_mid us_ret where
      u_steps: "U_D3 p (x_var s p) (s_var s p) (snd s) us_mid"
               ...
               "U_D5 p us_ret (snd s')"
      unfolding Sys_D4_def bridge by blast
    thus "u_program_counter (snd s') q = u_program_counter (snd s) q"
      using qp unfolding U_D3_def U_D4_def U_D5_def
      by auto
  qed

  let ?d4_ret = "mk_act deq (x_var s p) p (s_var s p) ret"

  have d4_ret_cr [simp]: "act_cr ?d4_ret = ret"
    by (simp add: mk_act_def act_cr_def)

  have his_append: "his_seq s' = his_seq s @ [?d4_ret]"
    using step_facts by simp

  have OpCalledInHis_append_ret_iff:
    "\<And>H e a. act_cr e = ret \<Longrightarrow> OpCalledInHis (H @ [e]) a = OpCalledInHis H a"
  proof -
    fix H e a
    assume e_ret: "act_cr e = ret"
    show "OpCalledInHis (H @ [e]) a = OpCalledInHis H a"
    proof
  ...
  have HB_append_ret_iff:
    "\<And>H e a b. act_cr e = ret \<Longrightarrow> HB (H @ [e]) a b = HB H a b"
  proof -
    fix H e a b
    assume e_ret: "act_cr e = ret"
    show "HB (H @ [e]) a b = HB H a b"
    proof
  ...
  have USpec_GenLin_append_ret:
    "\<And>H e S oid L. act_cr e = ret \<Longrightarrow>
       USpec_GenLin H S oid L \<Longrightarrow> USpec_GenLin (H @ [e]) S oid L"
  proof -
    fix H e S oid L
    assume e_ret: "act_cr e = ret"
    assume GEN: "USpec_GenLin H S oid L"
    have calls: "\<forall>a\<in>set L. OpCalledInHis (H @ [e]) a"
      using GEN OpCalledInHis_append_ret_iff[OF e_ret]
    ...
    proof (unfold HB_consistent_def, intro allI impI)
      ...
      hence "HB H (L ! k1) (L ! k2)"
        using HB_append_ret_iff[OF e_ret, of H "L ! k1" "L ! k2"] by simp
      thus "k1 < k2"
        using GEN pre unfolding USpec_GenLin_def HB_consistent_def by blast
    qed
    show "USpec_GenLin (H @ [e]) S oid L"
      using GEN calls hb unfolding USpec_GenLin_def by blast
  qed

  have HB_his_stable [simp]: "\<And>a b. HB (his_seq s') a b = HB (his_seq s) a b"
    using HB_append_ret_iff[OF d4_ret_cr, of "his_seq s"] his_append by simp

  have modify_his_stable [simp]:
    "\<And>L v. modify_lin L (his_seq s') v = modify_lin L (his_seq s) v"
  proof -
    fix L v
    show "modify_lin L (his_seq s') v = modify_lin L (his_seq s) v"
    proof (rule modify_lin_HB_stable)
      fix a b
      show "happens_before a b (his_seq s') =
            happens_before a b (his_seq s)"
        using HB_append_ret_iff[OF d4_ret_cr, of "his_seq s" a b]
              his_append
        by simp
    qed
  qed
  ...
  proof (unfold uI2_USpec_E1UE2_def, intro allI impI)
    ...
    have new_gen:
      "USpec_GenLin (his_seq s') (uspec_effOps s')
        (mk_op enq (v_var s' q) q (s_var s' q))
        (lin_seq s' @ [mk_op enq (v_var s' q) q (s_var s' q)])"
      using USpec_GenLin_append_ret[OF d4_ret_cr old_gen] his_append enq_eq
      by simp
  ...
  proof (unfold uI3_USpec_D3UD2_def, intro allI impI)
    ...
    proof (cases "should_modify (lin_seq s) (his_seq s) ?x_old")
      ...
      have new_true:
        "should_modify (lin_seq s') (his_seq s') ?x_new"
        using should_eq True
        by simp

      have base_new_eq:
        "?base_new = modify_lin (lin_seq s') (his_seq s') ?x_new"
        using new_true
        by (simp only: if_True)

      have base_old_eq:
        "?base_old = modify_lin (lin_seq s) (his_seq s) ?x_old"
        using True
        by (simp only: if_True)
    ...
    next
      ...
      have new_false:
        "\<not> should_modify (lin_seq s') (his_seq s') ?x_new"
        using should_eq False
        by simp
    ...
    have new_gen0:
      "USpec_GenLin (his_seq s') (uspec_effOps s) ?op_old ?L_old"
      using USpec_GenLin_append_ret[OF d4_ret_cr old_gen] his_append
      by simp
    ...
    show "let cur_lin = lin_seq s';
          ...
          in USpec_GenLin cur_his (uspec_effOps s') op new_lin"
      using new_gen0
      by (simp only: Let_def uspec_effOps_eq op_eq L_eq_oldop)
  qed
