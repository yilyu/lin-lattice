...
* the Herlihy-Wing queue (HWQ) is simulated by $U_{Queue}$;
...
* `HWQ-U/`: the HWQ-to-$U_{Queue}$ development, together with queue-specification, history-transfer, $U_{Queue}$-membership, and final simulation results;
...
* `E1` (`C_E1` / `Sys_E1`) reserves a slot by reading/incrementing `X`; the abstract $U_{Queue}$ state stutters and `lin_seq` is unchanged.
...
This directory contains the Isabelle/HOL development for the HWQ-to-$U_{Queue}$ proof.

The main files are:

* `Model.thy`: concrete/abstract states, histories, linearization sequences, invariants, and HWQ/$U_{Queue}$ transition rules;
...
* `ULinProof.thy`: core HWQ/$U_{Queue}$ simulation and recorded-history linearizability results;
...
* `USpecMembershipLemmas.thy`, `USpecMembership.thy`: $U_{Queue}$ membership consequences of the invariant;
* `HWQQueueLemmas.thy`, `HWQQueueProof.thy`: final HWQ-to-$U_{Queue}$ theorem layer, including `HWQ_is_CR_simulated_by_UQueue`.
...
* HWQ is simulated by $U_{Queue}$: `HWQ-U/HWQQueueProof.thy`;
...
The `ROOT` session also includes the queue-specification, history-transfer, and $U_{Queue}$-membership layers used by the revised HWQ-U development.

## Correspondence to the paper

* **$U_{Queue}$ and the Herlihy-Wing queue case study:** mainly `HWQ-U/`.
