...
- the Herlihy-Wing queue (HWQ) is simulated by U<sup>s</sup><sub>Queue</sub>;
...
- `HWQ-U/`: the HWQ-to-U<sup>s</sup><sub>Queue</sub> development, together with queue-specification, history-transfer, U-specification-membership, and final simulation results;
...
- `E1` (`C_E1` / `Sys_E1`) reserves a slot by reading/incrementing `X`; the abstract U<sup>s</sup><sub>Queue</sub> state stutters and `lin_seq` is unchanged.
...
This directory contains the Isabelle/HOL development for the HWQ-to-U<sup>s</sup><sub>Queue</sub> proof.

The main files are:

- `Model.thy`: concrete/abstract states, histories, linearization sequences, invariants, and HWQ/U<sup>s</sup><sub>Queue</sub> transition rules;
...
- `ULinProof.thy`: core HWQ/U<sup>s</sup><sub>Queue</sub> simulation and recorded-history linearizability results;
...
- `USpecMembershipLemmas.thy`, `USpecMembership.thy`: U-specification membership consequences of the invariant;
- `HWQQueueLemmas.thy`, `HWQQueueProof.thy`: final HWQ-to-U<sup>s</sup><sub>Queue</sub> theorem layer, including `HWQ_is_CR_simulated_by_UQueue`.
...
- HWQ is simulated by U<sup>s</sup><sub>Queue</sub>: `HWQ-U/HWQQueueProof.thy`;
...
The `ROOT` session also includes the queue-specification, history-transfer, and U-specification-membership layers used by the revised HWQ-U development.

## Correspondence to the paper

- **U<sup>s</sup><sub>Queue</sub> and the Herlihy-Wing queue case study:** mainly `HWQ-U/`.
